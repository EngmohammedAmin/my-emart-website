...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $(document).on("click", '.add-to-cart', async function(event) {
            ...
            setCookie('vendor_longitude', vendor_longitude, 365); setCookie('vendor_latitude',
                vendor_latitude, 365);

            if (getCookie('service_type') == "Ecommerce Service") {

                await database.collection('sections').doc(section_id).get().then(async function(
                    deliveryChargeSnapshots) {
    ...
    async function getVendorDetails(vendorID) {
        ...
        vendorDetailsRef.get().then(async function(vendorSnapshots) {
            ...
            $("#seller-image").html('<a href="' + view_vendor_details +
                '"><img style="height: 65px; width: 65px; border-radius: 50%" src="' + photo +
                '"></a>');
                                ...
                                for (j = 0; j < specialOfferVendor[i]['timeslot'].length; j++) {
                                    if (currentTime >= specialOfferVendor[i]['timeslot'][j]['from'] &&
                                        ...
                                        if (specialOfferVendor[i]['timeslot'][j]['discount_type'] ==
                                            'delivery') {
                                            specialOfferForHour = [];
                                            specialOfferForHour.push(specialOfferVendor[i]['timeslot'][
                                                j
                                            ]);
                                        }
                                    }
    ...
    function getVendorProducts(vendorID) {
        database.collection('vendor_products').where("vendorID", "==", vendorID).where("publish", "==", true).get()
            .then(async function(snapshots) {
                var total = snapshots.docs.length;
                $(".vendor-total-product").text(total);
            });
        database.collection('vendor_products').where("vendorID", "==", vendorID).where("publish", "==", true).where(
            'id', "!=", id).limit(5).get().then(async function(snapshots) {
            var html = '';
        ...
        if (vendorProduct != undefined) {
            ...
            if (vendorProduct.hasOwnProperty('addOnsPrice') && vendorProduct.addOnsPrice.length > 0) {
                ...
                vendorProduct.addOnsPrice.forEach(async (product_price) => {
                    ...
                    html += '<input data-price="' + product_price + '" type="checkbox" id="' + vendorProduct
                        .id + '_extra_' + total + '" name="extra_' + vendorProduct.id + '" value="' +
                        vendorProduct.addOnsTitle[total] + '" class="custom-control-input extra_' +
                        vendorProduct.id + '">';
                    html += '<label class="custom-control-label" for="' + vendorProduct.id + '_extra_' +
                        total + '">' + vendorProduct.addOnsTitle[total] + ' <span class="">+$' +
                        product_price + '</span></label>';
                    html += '</div>';
                    html += '</div>';
                    total++;
                });
            ...
            html += '<input type="hidden" id="category_id_' + vendorProduct.id + '" name="category_id_' + vendorProduct
                .id + '" value="' + vendorProduct.categoryID + '">';
